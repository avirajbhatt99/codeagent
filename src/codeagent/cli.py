...
app = typer.Typer(
    name="codeagent",
    help="ultracode — AI-powered coding assistant CLI",
    add_completion=True,
...
def run_setup_wizard() -> StoredConfig:
    ...
    console.print("\n[bold cyan]Welcome to ultracode![/bold cyan]\n")
...
def print_welcome(model: str, path: str) -> None:
    ...
    console.print()
    console.print("[bold bright_magenta]  ◆ ultracode[/bold bright_magenta]", end="")
...
def start_session(verbose: bool = False) -> None:
    ...
    try:
        ...
        console.print("[dim]Run: ultracode setup[/dim]")
        return
    except Exception as e:
...
def main(
    ...
    if version:
        console.print(f"ultracode v{__version__}")
        raise typer.Exit()
...
def models(
    ...
    if target == "ollama":
        ...
        if not models_list:
            console.print("[yellow]No models installed[/yellow]")
            console.print("[dim]Download: ultracode pull[/dim]")
        else:
    ...
    console.print(f"\n[dim]Change: ultracode config --model <name>[/dim]\n")
